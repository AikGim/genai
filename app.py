...
def key():
    ...
    if flag==1:
        ...
        flag=0
    return(render_template("key.html",r=name))

@app.route("/check",methods=["GET","POST"])
def check():
    print(os.environ['REPLICATE_API_TOKEN'], palm)

@app.route("/keyconfig",methods=["GET","POST"])
def keyconfig():
    rkey = request.form.get("rkey")
    mkey = request.form.get("mkey")
    os.environ['REPLICATE_API_TOKEN'] = rkey
    palm.configure(api_key=mkey)
    return(render_template("main.html",r=name))
